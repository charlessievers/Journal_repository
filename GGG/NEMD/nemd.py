...
if __name__ == "__main__":
    ...
    lmp.command("region ral    block -100 100 -100 100 33 39 ")
    lmp.command("region right  block -100 100 -100 100 40 46 ")
    lmp.command("region redge  block -100 100 -100 100 52 53 ")
    lmp.command("region system block -100 100 -100 100 2  52 ")
    ...
    for i in range(num_pico_s):
        for j in range(100):
            ...
            if j > 89:
                ...
                five += lmp.extract_compute("lal_temp", 0, 0)
                six += lmp.extract_compute("C0_temp", 0, 0)
                seven += lmp.extract_compute("C1_temp", 0, 0)
                nine += lmp.extract_compute("C2_temp", 0, 0)
                eight += lmp.extract_compute("ral_temp", 0, 0)
        ...
        if rank == 0:
            ...
            flux[i] = [hot, cold]
            t_profile[i] = [left/10, lal/10, C0/10, C1/10, C2/10, ral/10, right/10]
            if i % 25 == 0:
