...
if __name__ == "__main__":
    ...
    lmp.command("region ral    block -100 100 -100 100 27 34 ")
    lmp.command("region right  block -100 100 -100 100 34 40 ")
    lmp.command("region redge  block -100 100 -100 100 45 47 ")
    lmp.command("region system block -100 100 -100 100 2  45 ")
    ...
    lmp.command("run 0")
    left = lal = C0 = ral = right = 0
    t_profile = np.zeros((num_pico_s, 5))
    ...
    for i in range(num_pico_s):
        ...
        if rank == 0:
            ...
            flux[i] = [hot, cold]
            t_profile[i] = [left/10, lal/10, C0/10, ral/10, right/10]
            if i % 25 == 0:
