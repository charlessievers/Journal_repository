...
lmp.command("region top block -100 100 -100 100 57 63")
...
for i in np.arange(-5, -4):
    ...
    lmp.command("dynamical_matrix all eskm 0.000001 file dynmat.dat binary yes")

    if rank == 0:
        dynmat = np.fromfile("dynmat.dat", dtype=float)
        dynlen = int(np.sqrt(len(dynmat)))
        # dynmat = np.loadtxt("graphite.dat")
