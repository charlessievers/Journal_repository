...
if __name__ == "__main__":
    ...
    lmp.command("region ral    block -100 100 -100 100 44 49")
    lmp.command("region right  block -100 100 -100 100 51 56")
    lmp.command("region redge  block -100 100 -100 100 62 64")
    lmp.command("region system block -100 100 -100 100 2  62")
    ...
    lmp.command("group MoS type 4 5")
    ...
    lmp.command("run 0")
    left = lal = C0 = M1 = M2 = C1 = ral = right = 0
    t_profile = np.zeros((num_pico_s, 8))
    ...
    for i in range(num_pico_s):
        ...
        if rank == 0:
            ...
            flux[i] = [hot, cold]
            t_profile[i] = [left/10, lal/10, C0/10, M1/10, M2/10, C1/10, ral/10, right/10]
            if i % 25 == 0:
