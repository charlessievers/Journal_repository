...
if __name__ == "__main__":
    ...
    lmp.command("region ral    block -100 100 -100 100 48 55")
    lmp.command("region right  block -100 100 -100 100 55 61")
    lmp.command("region redge  block -100 100 -100 100 67 68")
    lmp.command("region system block -100 100 -100 100 2  67")
    lmp.command("region half   block -100 100 -100 100 10 32")
    lmp.command("group Al type 1")
    lmp.command("group C0 type 2")
    lmp.command("group C1 type 3")
    lmp.command("group C2 type 4")
    lmp.command("group MoS type 5 6")
    ...
    lmp.command("group half region half")
    lmp.command("group system region system")
    lmp.command("group edge union ledge redge")
    lmp.command("group MoS1 intersect MoS half")
    lmp.command("group MoS2 subtract MoS half")
    ...
    lmp.command("compute MoS1_temp MoS1 temp")
    lmp.command("compute C1_temp C1 temp")
    lmp.command("compute MoS2_temp MoS2 temp")
    lmp.command("compute C2_temp C2 temp")
    lmp.command("compute ral_temp ral temp")

    lmp.command("run 0")
    left = lal = C0 = M1 = C1 = M2 = C2 = ral = right = 0
    t_profile = np.zeros((num_pico_s, 9))
    ...
    for i in range(num_pico_s):
        for j in range(100):
            ...
            if j > 89:
                ...
                M1 += lmp.extract_compute("MoS1_temp", 0, 0)
                C1 += lmp.extract_compute("C1_temp", 0, 0)
                M2 += lmp.extract_compute("MoS2_temp", 0, 0)
                C2 += lmp.extract_compute("C2_temp", 0, 0)
        ...
        if rank == 0:
            ...
            flux[i] = [hot, cold]
            t_profile[i] = [left/10, lal/10, C0/10, M1/10, C1/10, M2/10, C2/10, ral/10, right/10]
            if i % 25 == 0:
